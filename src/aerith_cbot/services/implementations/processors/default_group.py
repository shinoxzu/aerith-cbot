...
from aerith_cbot.services.abstractions import GroupMessageProcessor, SenderService
...
class OpenAIGroupMessageProcessor(GroupMessageProcessor):
    def __init__(
        self,
    ...
    async def process(self, message: types.Message) -> None:
        ...
        if not chat_state.is_focused:
            ...
            is_triggered = await self._is_message_trigger_focus(message)
        ...
        try:
            new_messages.append(
                {
                    "role": "user",
                    "content": tg_msg_to_model_input(message).model_dump_json(exclude_none=True),
                },
            )
            ...
            # and try to send llm text response to the user
    ...
    async def _is_message_trigger_focus(self, message: types.Message) -> bool:
        is_triggered = False
